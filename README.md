...
- **🚚 跨地区规划** —— 四号谷地 ⇄ 武陵的货运建议与「哪个成品放哪片基地更省」（2^N 组合穷举，成本逐项可解释）
- **🎯 最优排布**（开发中）—— 选定目标物品，按「用电 → 占地 → 物流线」自动分配到该地区各基地并生成摆法
...
> ⚠️ **对外分享只写上面这条 `workbuddy.link/p/` 短链**（免登录、URL 永不变）。其他任何形式（登录态入口、含版本号的静态直链）都**不要写进文档、README、聊天记录或任何长期留存的地方**——详见 [docs/维护与更新.md](docs/维护与更新.md) 的「分享链接口径」。
...
**本项目代码**采用 [MIT License](LICENSE) —— 可自由使用、修改、分发（保留版权声明即可）。

**游戏数据**（物品名、配方、数值、文案等）版权归**鹰角网络及相关权利方**所有。本项目仅供学习、交流与研究使用，
不得用于侵犯权利方权益或其他非法用途；仓库**不包含**任何游戏解包原始表（`raw/` 不入版本库），
只包含由公开数据域提取的结构化产物。

**数据来源**：[AKEDatabase](https://github.com/NagiYume/AKEDatabase)（数据域 `data.akedata.wiki`，
该项目代码采用 AGPL v3.0）—— 本项目**未复制其代码**，仅按其公开的数据域路径规则拉取数据表。

**规则调研**：分流/汇流/桥接等物流生成规则参考了
[IndustrialPlanner](https://github.com/hsyhhssyy/IndustrialPlanner)（MIT）——
调研记录见《IP源码规则提炼.md》，**未复制其代码**。
