...
// 主/副归属：每个据点 1 主 + 3 副，主基地是枢纽区 / 武陵城（作者游戏内确认，写死在这里守着）
chk('主基地落在枢纽区与武陵城',
...
// 反向锁：「flex 定宽栏」那条错路必须已撤掉 —— 它会让展开时画布整体右移（作者：「画布又被挪了」）
...
// 「功能设备」（v131 前旧名「物流辅助」）下的建筑被作者逐个点名拉黑（洒水机/给水器/滑索架/便捷存取站/留言信标），
...
// 滑索架（travel_pole_1 / travel_pole_2 / travel_pole_nop_1）已被作者点名拉黑，不再进默认清单
...
chk('枢纽区是相连两条边排满（作者实拍）', !!busBy['map01_lv001'] && busBy['map01_lv001'].edges === 2);
chk('枢纽区有源桩、副基地没有（作者 2026-09-21 补充）',
...
// 作者：「布局试摆我看不见四号谷地的预设存取线，能把武陵和四号谷地分开讨论吗」。
...
// ---- 5d-7. 谷地预设存取线：贴在画布**外缘**、不占格（作者 2026-09-21 定）----
// 原话「就是贴在画布外缘，不占基地格子」→ 整条带子画在画布框外面（负偏移），一格都不占。
// 摆法照基地面积页那张示意图（作者：「就按这张示意图画」）：枢纽区 = 源桩占左上角 + 上边/左边铺满；副基地 = 左上一条边。
...
// ⭐v145 反向锁（2026-09-24 作者游戏内确认「核心区无限制」）：
...
chk('第2期 RbaseBest：谷地不启销毁专区（作者：新手村不做）', (() => {
  const rb = A.RbaseBest([{ id: 'item_filter_core', name: '分离芯', rate: 10 }], '四号谷地');
...
/* ═══ v159（2026-09-25）判据同源 + 失败隔离 ═══
   背景：作者实测「一堆产线自动生成不出来」。根因 = RbaseBest 只看容量不查台数上限，
...
/* v159.1（2026-09-25 作者选①）：落画布后视线**切到首个落点基地**。
   原行为是固定切回作者原来的基地 → 落点不在原基地时看到空画布（以为没生效）。 */
...
/* ═══ v160（2026-09-25 作者拍板方案 B）：切换目标物品 → 该地区基地产线自动跟着换 ═══
...
chk('第3期 RxlAll：多地可选的目标给出「为什么去这里」（点出另一处的收货/矿缺口，R3）', (() => {
  /* 分离芯：两地的封装机都能建（不该标 only），但谷地缺赤铜矿（满采上限 0）
     且要收息壤粉 → 理由里必须点出这个代价（作者 2026-09-25 追问核实的事实）。 */
...
/* LgenAll / LapplyAssignAll：跨地区落画布（自由模式起手 → 落盘 → 一次撤销）
   ⚠️ 目标用「铁制零件」——探针实测它跨地区择优落到**四号谷地**（枢纽区）。
      ...
      卡住谷地的是**原料**（赤铜矿），不是机器（作者 2026-09-25 追问后实测核实）。 */
A.LbaseSet('');
...
/* ═══ 第 4 期（v167，作者选「同地区基地间 + 跨地区都做」）：跨基地转运清单 + 带宽校验 ═══
...
// ---- 5d-8. 生产配方：设施选物品 + 产能配比（作者 2026-09-21）----
...
// ---- 5d-9. 产线闭环 · 排布器 v1（作者 2026-09-21：「做排布器，先做一次产线闭环」+「连连线也自动」）----
...
// ⑤b 管道连线（2026-09-21 作者：「先把管道连线打通」）——两个真 bug 都在这块
...
// —— 作者 2026-09-24 游戏实锤）。机器（非物流件）与任何件同格仍非法。返回违规格列表。
...
// ---- 5d-10. 「闭环自持」开关 + 多台并联（分流/汇流）（作者 2026-09-21）----
...
// ---- 5d-12. 第 1 层：约束硬校验（协议容量 / 发电 / 野外采集上限）（作者 2026-09-21）----
// 作者口径：矿脉纯度**按当前版本地区最大值**算（= 高纯度 20/分）；发电量用已给的社区数值；
...
chk('矿点清单：源矿 60（实测：四号谷地 28 + 武陵 32，作者截图 + 多方实测）/ 紫晶矿 12 / 蓝铁矿 60 个矿点', (() => {
...
// 作者要核的那张表：四号谷地有四方一致的实测值（560/240/1080），武陵是推算
...
// ---- 5d-11. 用电统计 + 野外开采产量（作者 2026-09-21）----
...
chk('开采数据：**水驱矿机在列**，且 desc 说明它采赤铜矿（作者 2026-09-21 抓到的漏项）', (() => {
...
// ⭐v108（作者图：「游戏里是从红圈里开始拉」）：机器的口格本身可以起手连铺 ——
...
// ⭐v122（作者截图「旋转个方向进出货口就不齐了」）：接口朝向必须跟着 rot 转。
...
// ⭐v123（作者「协议核心还是无法从机器口拉传送带，鼠标一移动到机器口上就只能选择物品」）：
...
// ⭐v124（作者「把选择物品的模块移到里面，外侧像其他基建一样是货品进出口」）：箭头内移
...
(function () {
  ...
  //    （作者实测「又选不了货了」）。守卫收窄为只拦物流件后，此场景必须恢复。
  A.Lpick('sp_hub_1');
...
// ⭐v106 弯头格渲染（作者 2026-09-23 游戏截图「一格拐弯画不了」）：
...
// ⭐v107（作者图1/图2，2026-09-23）：① 拐弯第一轴跟手势轨迹（先往上拖就先铺竖段），
...
// ⭐v109 第二半（作者 2026-09-23：「游戏里的协议核心出货口可以点击选择物品出货」+
...
(function () {
  ...
  // ⭐v127 瓶罐筛选 + 灌装物标注（作者「你这里全是一样的瓶罐」——紫晶质瓶同名 ×10+，
...
// ⭐v128 链尾自动拧转（作者 2026-09-24「连续放传送带时，在上一条传送带的末尾拐弯放置，
...
// ================= ⑤-1 局部锁定（作者 2026-09-22「锁住满意的机器，只重排其余」）=================
// ① 标记 / 渲染 / 撤销
loReset(50);
...
// ⭐2026-09-24 阈值 ≤2 → ≤3（作者委托处理）：对账 v150(d050390)/v151(a68cb07)/v152 工作区
...
// 放开后（v152），RwPath 里异介质交叉成本可从 BRIDGE=4 下调（改前先给作者过目）。
...
// ================= 采集建筑的「无线回传」标记（2026-09-22 作者问的）=================
// 作者：「矿机为什么会有出货口啊，矿机是挖完了把矿物无线传回基地仓库」。
...
//    它是作者 2026-09-22 实机确认「也无线回传」的 —— 数据里必须写明来源，别让它看起来像配置表读出来的。
...
// 作者追问「那这样 4 还用吗，玩家都是用无线传输的」→ 那 3 个出料口两种模式都用不上，**示意图干脆不画**，
// 改成在建筑上标「⇡ 无线回传仓库」徽标。
...
chk('水驱矿机：也标「无线回传仓库」（作者实机确认），出料口不画、但清水管道进料口照常画',
    m4Svg.indexOf('无线回传仓库') >= 0 && m4Svg.indexOf('>O<') < 0 &&
    m4Svg.indexOf('>i<') >= 0 && m4Html.indexOf('管道进清水') >= 0 && m4Html.indexOf('作者实机确认') >= 0);
...
// 作者追问「那这样 ④ 还用做吗」——答：不用按原样做。因为三种矿机的产物都无线回仓/进缓存区，
...
// 作者口径：「只用从四号谷地向武陵超库存传输就行，我基地等级已经升满了可以超库存传输」
...
// ---- ⑥-1 满级口径（作者 2026-09-22 定：只要超库存、按满级）—— 开收货时预填 1500 ----
A.Linit(); A.LO.tv = 0;
A.LshipIn();
...
// ---- ⑥-1 单选（2026-09-22 作者指出 + GameRant/GameWith/Game8 三源核实）----
...
chk('⑥-1 v89 瓶罐装什么可见（作者：「瓶罐里装的什么我看不到」）：选货卡带构建期反推的 content 标注——赤铜耐压罐/赤铜瓶/蓝铁瓶等同名变体各自显示装的内容物，空容器标「空容器（可灌装）」',
    (() => { A.LO.shipIn = true; A.LO.tv = 1500; A.LO.shipPick = ''; A.LawRun('item_proc_battery_2', 1);
      const h2 = outEl.innerHTML;
      const iF = h2.indexOf('<details class="lo-pickfold"');
      const fold = iF >= 0 ? h2.slice(iF) : '';
...
chk('⑥-1 v82 换目标即刷新候选（作者截图：旧链候选还挂着）：电池链 → 石英玻璃链，shipChain 与默认选中跟着换',
    (() => { A.Linit(); A.LO.objs = []; A.LO.sel = []; A.LO.plan = null;
...
// ---- v134 反应池面板：缓存格推演 + 池子判定（作者 2026-09-24 要「像游戏那样显示缓存槽」）----
...
// ---- v135「点机器就地选」框架（作者 2026-09-24：「我要在这里选，要做到以后能逐步完善到其他基建都能在这里选」）----
...
// ---- v136 对标调研 D + A（作者 2026-09-24「先做 DA」）----
...
// ---- v137 手动连铺十字相交自动建桥（作者：「游戏里两条传送带相交后会自动建物流桥」）----
...
// ---- v138 准入口（作者：「准入口只可以放在传送带和管道上」+ 准入物品 + 限速）----
...
// ---- v139 准入口合规标红（作者：「拐角也是怎么还放得下」→ 已有的不合规件也要标出来）----
...
// ---- ⑥-4 建筑专属限摆：天有洪炉 ≤12 台（2026-09-22 作者拍板 + 查证）----
...
// ---- 💨 v103 气体散布机环境圈（2026-09-23 作者拍板：方形不是圆）----
...
(function () {
  ...
  chk('💨 v104 校准色：env1-4 → 稳定青蓝/湿润白/酸性橙黄/息壤翠绿（作者截图口径）',
...
// ---- v151 外部流体接入口（2026-09-24 作者定稿）----
// 作者截图实锤：实际玩法是拉很多根水管分别供给多台设备 ——「每流体一条」的收窄版已回退，
...
// ④ v151 续2：feed 起点格（暗管接入点）进边=朝画布外那侧（feedStart 表）——作者截图
...
// ---- v152 管×带交叉不放假桥（作者 2026-09-24 游戏实锤：3D 里管道在上层、传送带在下层，
...
// ---- v154 暗管入口/出口对（作者 2026-09-25 实机规则：一对一定向、同建筑同物料、可旋转）----
...
// ---- C6-b 阶段 2：门禁 + 自动补销毁支线（2026-09-25 作者拍板「有现成消费者就接、否则销毁 + 软门禁」）----
