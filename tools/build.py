...
#   游戏内「工业设备」面板的分组与它对不上（作者 2026-09-24 截图指出：仓库存取线基段和源桩应同组）。
...
def main():
    ...
    # ⚠️ 2026-09-22（作者实机核对后揪出）：furnance_nop_1（精炼炉免电变体）原始表里
    ...
    for bid, b in buildings.items():
        ...
        rec = {
            ...
            #   但作者 2026-09-24 实机放置预览是 5×5（且两池接口坐标完全相同）→ 判为配置表口径偏差。
    ...
    for b in building_list:
        if not (b.get("gridFootprint") and b["gridFootprint"] != "None×None"):
            continue
        # ⚠️ 2026-09-22（作者拍板，v93）：剔除储液罐免电版——它与普通储液罐在蓝图页
    ...
    # ★ 等级上限（LevelGradeTable）—— 2026-09-21 深夜补（作者问「协议容量的版本等级限制」）
    ...
    # ⚠️ 矿机**全都**不靠出料口接带子（作者 2026-09-22 两次指出，第二次纠的就是水驱矿机）：
    #   野外矿点采**固体矿**，而野外铺不了传送带 → 产物只能「自动回仓」或「人工取」。
    #   三种矿机：电驱/二型电驱（无线，配置表 hasDroneMode）+ 水驱（无线，作者实机确认、配置表无该字段）
    #   + 便携源石矿机（不无线，进缓存区、手动取）。
    ...
    WIRELESS_NOTE = {
        ...
        # ⚠️ 版本口径（作者 2026-09-22 纠正 + 联网核实）：水驱矿机是 **1.1「新潮起，故渊离」（武陵工业二期）** 新增的，
    ...
    # 无线这条结论的来源分级（配置表能取到的就写配置表；取不到的是作者实机确认）
    ...
    for b in building_list:
        ...
        row = {
            ...
            #    按作者 2026-09-22 的实机确认补上（野外采固体矿、铺不了传送带 → 必然自动回仓）。
    ...
    #    作者 2026-09-21 定：「按当前版本地区最大值算」→ **取高纯度 20/分**，正好等于配置表算出的 20/分（msPerRound 3000），两个口径自洽。
    #
    # ⚠️⚠️ 2026-09-21 晚**二次核查**（作者问「各地图矿区真的都查干净了吗」），三处修正：
    ...
    # ⚠️⚠️ 2026-09-21 晚**三次核查**（作者：「矿源点矿产纯度协议容量什么的版本等级限制再查一遍，
    #    留个接口给以后游戏版本更新用，矿源点每张小地图都列出来」+「赤铜矿理论最大开采值是 510/分」）：
    #   ① **作者给的 510 是对的，我上一版算的 160 是根本性错误** —— 把「清波寨一个区」当成了全图。
    ...
    #   ③ **1.5 全图满采 = 510/分**（游戏内「理论最大开采值」；作者确认 + NGA tid=47560240 印证）。
    ...
    # ⚠️⚠️⚠️ 2026-09-21 **第四次核查 —— 这次推翻了我自己的核心算法**（作者：「你可以自己查，
    ...
    #   ③ **按大地区的最大理论值**（作者要核实的那张表）放在 `mapMax`：
    ...
    #      **供能高地要到 11 级**；武陵 **景玉谷 8 级**满纯。本库按作者定的「地区最大值」口径 → 一律按高纯度 20/分。
    ores = {
        "schemaVersion": 3,
        ...
        "beds": [
            ...
            {"ore": "赤铜矿", "itemId": "item_copper_ore",
             ...
             # ✅✅ 2026-09-21 深夜（终版·闭环）：作者给了武陵简报截图，逐区计数（含高低纯度）：
    ...
    # 电池燃料按地区分（作者 2026-09-21 定：谷地用谷地电池、武陵用武陵电池）
    fuel_by_region = {
