...
def main():
    ...
    WIRELESS_NOTE = {
        ...
        "miner_4": "⚡ **产物同样回仓库**（博士 2026-09-22 实机确认；配置表里 `miner_4` **不在矿机表**中，没有 `hasDroneMode` 字段）。"
    ...
    WIRELESS_SRC = {"miner_2": "配置表", "miner_3": "配置表",
                    "miner_4": "博士实机确认（配置表里无该字段）", "miner_1": "配置表"}
    WEB_SOURCES = [
    ...
    power = {
        ...
        "generation": {
            ...
            "warn": "⚠️ 社区数值，未用配置表交叉验证（配置表根本没有这一项）。要当硬约束用，建议博士在游戏里核一下。",
            "sources": [
    ...
    ores = {
        ...
        "purityRule": {
            ...
            "adopted": "high",
            "adoptedWhy": "博士 2026-09-21 定：按当前版本**地区最大值**算 → 用高纯度 20/分（= 配置表 msPerRound 3000 的值，两口径自洽）",
        ...
        "beds": [
            {"ore": "源矿", "itemId": "item_originium_ore",
             ...
             "theoreticalMax": {"value": 1100, "version": "1.5",
                                "how": "四号谷地 28×20=560 + 武陵 22×20+10×10=540 = 1100/分（博士武陵简报截图逐区计数）"},
             "pointsNote": ("官方 FAQ 说全图 58 个矿脉；实测（博士武陵简报截图 + 四号谷地四方实测）= 28 + 32 = **60 个点**。"
             ...
             "mapMaxConfidence": {"四号谷地": "实测（NGA 两帖 + 游民星空 + sticweb 四方一致）",
                                  "武陵": "实测（博士武陵简报截图逐区计数：22 高×20 + 10 低×10 = 540，与 UI 一致）"},
             "zones": [
                 ...
                 {"zone": "景玉谷", "levelId": "map02_lv001", "points": 14, "high": 7, "low": 7, "perMin": 210,
                  "note": "武陵出生点区域；7 高 + 7 低（博士武陵简报截图）"},
                 {"zone": "武陵城", "points": 16, "high": 13, "low": 3, "perMin": 290,
                  "note": "13 高 + 3 低（博士武陵简报截图）；武陵主城，供电最方便"},
                 {"zone": "试验园区", "levelId": "map02_lv005", "points": 2, "high": 2, "low": 0, "perMin": 40,
                  "note": "2 个高纯度源矿点（博士武陵简报截图）；该区另有 2 个赤铜矿点"},
             ],
             "zonesCover": "四号谷地+武陵", "zonesCoverAll": True,
             "zonesSum": {"points": 60, "perMin": 1100, "version": "1.5"},
             "zonesNote": "按区明细只覆盖四号谷地 6 个建造区（合计 28 点 = 560/分，与 NGA 实测的「四号谷地源矿 560/分」完全吻合）；武陵 3 区明细来自博士武陵简报截图（景玉谷 7高7低 / 试验园区 2高 / 武陵城 13高3低）。",
            ...
            {"ore": "蓝铁矿", "itemId": "item_iron_ore",
             ...
             "zones": [
                 ...
                 {"zone": "武陵城", "points": 6, "high": 6, "low": 0, "perMin": 120,
                  "note": "6 个高纯度蓝铁矿点（博士武陵简报截图）= 120/分"},
             ],
             "zonesCover": "四号谷地+武陵", "zonesCoverAll": True,
             "zonesSum": {"points": 60, "perMin": 1200, "version": "1.5"},
             "zonesNote": "四号谷地 54 点 = 1080/分（与实测吻合）；矿脉源区也产蓝铁矿但那一区没有按区数字，未列入。供能高地桥头区要解「离群之狼」支线才进得去。武陵：武陵城 6 高 = 120/分（博士武陵简报截图）。",
            ...
            {"ore": "赤铜矿", "itemId": "item_copper_ore",
             ...
             "pointsNote": ("1.5 实测（博士武陵简报截图逐区计数）：23 个高纯度点 + 5 个低纯度点 = 28 点。"
                            "高纯度 20/分、低纯度 10/分 → 23×20 + 5×10 = **510/分，与游戏内 UI 完全一致，闭环**。"),
             "theoreticalMax": {"value": 510, "version": "1.5",
                                "how": "23 高 × 20 + 5 低 × 10 = 510（博士武陵简报截图逐区计数，与 UI 一致）",
                                "source": "博士武陵简报截图；NGA tid=47560240 同值"},
             "byMap": {"四号谷地": 0, "武陵": 28},
             "mapMax": {"四号谷地": 0, "武陵": 510},
             "mapMaxConfidence": {"武陵": "实测闭环 —— 博士武陵简报截图逐区计数，与游戏内 UI 完全一致"},
             "zones": [
        ...
        "versionLog": [
            ...
            {"version": "1.5", "note": "赤铜矿 28 点（23 高 + 5 低）= 510/分、源矿全图 60 点 = 1100/分，均与游戏内 UI 一致（博士武陵简报截图逐区计数，闭环）；新增雪凇幽梦地图（雪松林等）"},
        ],
        "sources": [
    ...
    power["fuelRule"] = ("博士 2026-09-21 定：**谷地用谷地电池、武陵用武陵电池**（源矿作为通用兜底）。"
